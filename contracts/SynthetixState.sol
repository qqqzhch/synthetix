...
contract SynthetixState is State, LimitedSetup {
     ...
     */
    function importIssuerData(address[] accounts, uint[] tUSDAmounts) external onlyOwner onlyDuringSetup {
        require(accounts.length == tUSDAmounts.length, "Length mismatch");

        for (uint8 i = 0; i < accounts.length; i++) {
            _addToDebtRegister(accounts[i], tUSDAmounts[i]);
        }
    }
